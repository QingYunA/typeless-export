...
import { detectIsEnglish, isEnglish, i18n } from '../src/i18n.mjs';
...
test('i18n and isEnglish work with environment', () => {
  assert.equal(isEnglish({ TLE_LANG: 'en' }), true);
  assert.equal(isEnglish({ TLE_LANG: 'zh' }), false);
  assert.equal(i18n('English', '中文', { TLE_LANG: 'en' }), 'English');
  assert.equal(i18n('English', '中文', { TLE_LANG: 'zh' }), '中文');
});
